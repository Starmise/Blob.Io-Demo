...
export class GameRoom extends Room {
  ...
  update(dt: number) {
    ...
    this.state.players.forEach((p: PlayerState) => { // Update invincibility and check pickups
      if (!p.isAlive) return;
  ...
  handleMove(sessionId: string, inputX: number, inputZ: number) {
    ...
    if (len === 0) return;
    const speed = BASE_SPEED / (1 + p.size * 0.15);
    const dt    = 1 / 20;
  ...
  checkPlayerCollisions() {
    ...
    for (let i = 0; i < players.length; i++) {
      for (let j = i + 1; j < players.length; j++) {
        ...
        if (dist < (a.size + b.size) * 0.4) {
          if      (a.size > b.size * 1.1) this.consumePlayer(a, b);
          else if (b.size > a.size * 1.1) this.consumePlayer(b, a);
        }
      }
    }
  }
  ...
  spawnBlobs(count: number) {
    for (let i = 0; i < count; i++) {
      ...
      blob.x     = (Math.random() - 0.5) * MAP_SIZE * 2;
      blob.z     = (Math.random() - 0.5) * MAP_SIZE * 2;
